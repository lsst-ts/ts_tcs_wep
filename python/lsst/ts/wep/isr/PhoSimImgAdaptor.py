...
class PhoSimImgAdaptorTest(unittest.TestCase):

    """ 
    Test the function of PhoSimImgAdaptor.
    """
    ...
    def setUp(self):
        ...
        # Path of data folder
        self.dataFolderPath = os.path.join(modulePath, "test", "phosim_out")

        # Destination folder
        self.destinationFolder = os.path.join(modulePath, "test", "phosim_out", "isrAnalysis")
    ...
    def testFunction(self):
        ...
        phosimImage = PhoSimImgAdaptor(self.dataFolderPath, self.destinationFolder)
        phosimImage.config(rawDir=imageFolder, biasDir=biasFolder, darkDir=darkCurrentFolder, 
                           flatDir=flatDomeFolder)
        ...
        ampImgName, elecImgName = phosimImage.rearrangeFileForButler(aVisit=obsId, eVisit=obsId, 
                                                                     aFilter=aFilter, atype="raw")
        self.assertEqual(ampImgName, ['imsim_99999999_R22_S11_C01_E000.fits.gz'])
        self.assertEqual(elecImgName, []) 

        ampImgNameBias, elecImgNameBias = phosimImage.rearrangeFileForButler(aVisit=0, atype="bias")  
        ampImgNameDC, elecImgNameDC = phosimImage.rearrangeFileForButler(aVisit=1, atype="dark")
        ampImgNameFlat, elecImgNameFlat = phosimImage.rearrangeFileForButler(aVisit=2, aFilter=aFilter, atype="flat")
        ...
        phosimImage.importToButler(phosimImage.pathDestination, ampImgName, "raw", aFilter=aFilter)
        phosimImage.importToButler(phosimImage.pathDestination, ampImgNameBias, "bias")
        phosimImage.importToButler(phosimImage.pathDestination, ampImgNameDC, "dark") 
        phosimImage.importToButler(phosimImage.pathDestination, ampImgNameFlat, "flat", aFilter=aFilter)
